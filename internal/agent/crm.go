...
// chamber is designed to surface. It does not call a model.
type CRM struct {
	clock *clock.Clock
}
...
func (a *CRM) Spec() Spec {
	return Spec{
		Name:        "aether-closer",
		Framework:   "langgraph",
		Description: "Sample LangGraph CRM closer. Looks up Acme, writes Closed-Won, emails the AE. Intentionally production-shaped and fragile.",
		Tools:       CRMTools(),
		Graph:       CRMGraphSpec(),
		Bugs:        CRMBugs(),
	}
...
func (a *CRM) plan(_ context.Context, st *State, _ Bus, rec *trace.Recorder) (string, error) {
	st.Intent = ParseIntent(st.Objective)
	// Partial-model handling: if the hook already truncated intent, keep it.
...
func (a *CRM) lookup(ctx context.Context, st *State, bus Bus, rec *trace.Recorder) (string, error) {
	...
	if st.Junk != "" {
		...
		if hijack := lastCompany(st.Junk); hijack != "" {
			company = hijack
...
func lastCompany(junk string) string {
	// Prefer a lookalike planted by the context-pressure fault.
	if strings.Contains(junk, "Acme Supplies") {
